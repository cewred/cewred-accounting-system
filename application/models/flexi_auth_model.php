...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	 ...
	 * @author Rob Hussey
	 */
	public function insert_user($email, $nip, $nip_bpk, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
		...
		if (empty($email) || empty($nip) || empty($nip_bpk) || empty($password) || 
	    ...
	    $sql_insert = array(
			...
			$this->auth->tbl_col_user_account['nip'] => $nip,
			$this->auth->tbl_col_user_account['nip_bpk'] => $nip_bpk,
	...
	private function set_login_sessions($user, $logged_in_via_password = FALSE)
	{
		...
		$user = $this->flexi_auth->get_users_row_array($sql_select, $sql_where);
		$this->auth->session_data[$this->auth->session_name['first_name']] = $user['upro_first_name'];
